...
async function fetchDriverDetails(driverId) {
    try {
        ...
        console.error(error);
        customDialog.showAlert("Lỗi!","Đã xảy ra lỗi khi tải thông tin tài xế.");
    }
}
...
async function updateDriverDetails(driverId) {
    try {
        ...
        customDialog.showAlert("Thành công!","Thông tin tài xế đã được cập nhật thành công.");
        window.location.href = "/BusManage/src/admin/manage_driver/manage_driver/manage_driver.html";
    } catch (error) {
        console.error(error);
        customDialog.showAlert("Lỗi!","Đã xảy ra lỗi khi cập nhật thông tin tài xế.");
    }
}
...
window.onload = () => {
    ...
    if (!driverId) {
        customDialog.showAlert("Lỗi!","Không tìm thấy ID của tài xế.");
        window.location.href = "/BusManage/src/admin/manage_driver/manage_driver/manage_driver.html";
        return;
    }
